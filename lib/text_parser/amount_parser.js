function amountParser (text, config) {
  ...
  for (var i = 0; i < parsers.length; i++) {
    parser = parsers[i]
    parserName = typeof parser === 'string' ? parser : (parser.name || 'custom')
    ...
    if (result.match) {
      result.strategy = parserName
      break
    }
  }

  result.confidence = getAmountConfidence(result)

  return result
}

function getAmountConfidence (result) {
  if (!result.match) return 0

  var confidence = result.strategy === 'total' ? 0.88 : 0.68
  var candidates = result.matches.filter(function (candidate) {
    return candidate.text === result.match
  })

  if (candidates.length > 1) confidence += 0.03

  var selected = candidates.length ? candidates[0] : null
  if (selected && selected.currency) confidence += 0.05
  if (selected && /[.,]\d{2}$/.test(selected.text)) confidence += 0.03
  if (selected && /\d[.,]\d{3}/.test(selected.text)) confidence += 0.01
